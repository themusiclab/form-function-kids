...
trial_data <- kfcFullDataset %>% 
  ...
  #select(user_id, 6:9, 12:15, 17:19, 21, 24) %>% 
  # add song number
  mutate(
    song = ifelse(
...
excl <- trial_data %>% 
  group_by(user_id) %>% 
  summarise(across(c(pilot, complete, missing), unique)) %>% 
...
exclusions$before_exclusions <- length(unique(excl$user_id))

# n excluded for not completing experiment
exclusions$non_complete <- excl %>% 
  filter(complete == 0) %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()

# n from pre-pilot thing
exclusions$pilot <- excl %>% 
  filter(pilot == 1) %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()
...
# n excluded from age
exclusions$age <- excl %>% 
  filter(age %in% c("13", "14", "14 or older", "15", "16", "17 or older")) %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()

# n from parent help
exclusions$parentHelp <- excl %>% 
  filter(parentHelp == "0") %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()

# n from played before
exclusions$playedBefore <- excl %>% 
  filter(playedBefore == "Yes") %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()

# n from hearing impairment
exclusions$hearing <- excl %>% 
  filter(hearingImpairment == "Yes") %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()

# n from missing data
exclusions$missing <- excl %>% 
  filter(missing == 1) %>% 
  summarise(n = n_distinct(user_id)) %>% 
  pull()
...
KFC_clean <- trial_data %>% 
  ...
  filter(pilot == 0,
         complete == 1,
         age %notin% c("13", "14", "14 or older", "15", "16", "17 or older"),
         parentHelp == 1,
         playedBefore != "Yes",
  ...
  select(user_id, rt, song, type, response, correct, age, country, language, playMusicOften, singOften, playedBefore, gender, headphones,
         parentHelp, hearingImpairment, likert) %>% 
  drop_na(age) %>% 
  mutate(age = ifelse(age == "3 or younger", "3", age),
         age = as.numeric(age),
...
FC_clean <- read_csv(file = here("data", "big", "fc_stimulusResponses_joined_processed.csv.gz"), 
  ...
  mutate(trialName = case_when(stimulus == "What is your gender?" ~ "gender",
...
# extract covariates
covs <- FC_clean %>% 
  filter(trialName %in% c("gender", "age", "country", "songType", "takenBefore")) %>% 
...
FC_clean <- FC_clean %>% 
  filter(trialName == "songType") %>% 
  select(user_id, created_at, correct, song, type, guess, likert) %>% 
  left_join(., covs) %>% 
...
save(KFC_clean, FC_clean, songs, exclusions, guesses, file = here("results", "preprocessed_data.RData"))
