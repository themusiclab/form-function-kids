...
f2 <- function(num) {
  formatC(num, 2, format="f") %>% format(., nsmall=0, big.mark=",")
}
...
To measure the degree to which these inferences change through development, we fit a simple linear regression predicting children's average accuracy from age, finding a small but significant effect (*F*(1, `r mods$accuracy$age$df.residual %>% f`) = `r mods$accuracy$age$statistic %>% f2`, *p* `r mods$accuracy$age$p.value %>% format_p()`, *R*^2^ `r mods$accuracy$age$r.squared %>% format_p()`), consistent with our exploratory analyses and preregistered hypothesis (i.e., a small, or practically non-significant increase in accuracy).
